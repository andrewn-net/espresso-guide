...
import { BookOpen, Plus, Trash2, Calendar, Gauge, Beaker, Timer, Info, Save, X, LogOut, Cloud, CloudOff } from 'lucide-react';
import { supabase } from '@/lib/supabase';

export default function BrewProfilesMode() {
    const { brewProfiles, addBrewProfile, removeBrewProfile, user, signOut, theme } = useStore();
    ...
    return (
        <div className="relative w-full min-h-[100dvh] bg-background text-foreground overflow-y-auto pt-16 pb-[140px]">
            ...
            <div className="relative z-10 w-full max-w-2xl mx-auto px-4 md:px-6 space-y-8">
                ...
                {/* Auth & Sync Status */}
                <div className="bg-secondary/40 backdrop-blur-xl rounded-2xl border border-border/50 p-3 md:p-4 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2 md:gap-3 min-w-0">
                        <div className={`p-1.5 md:p-2 rounded-full flex-shrink-0 ${user ? 'bg-emerald-500/10 text-emerald-500' : 'bg-orange-500/10 text-orange-500'}`}>
                        ...
                        <div className="min-w-0">
                            ...
                            <p className="text-[9px] md:text-[10px] text-muted-foreground uppercase tracking-wider leading-tight">
                                {user ? user.email : 'Sign in to save to cloud'}
                            </p>
                        </div>
                    </div>
                    {user ? (
                        <button
                            onClick={signOut}
                            className="flex items-center gap-2 px-3 py-1.5 bg-secondary hover:bg-secondary/80 rounded-xl text-xs font-bold transition-all whitespace-nowrap"
                        >
                            <LogOut size={14} />
                            Sign Out
                        </button>
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 md:gap-3 px-3 md:px-4 py-2 md:py-2.5 rounded-full text-[13px] md:text-sm font-medium shadow-md transition-all whitespace-nowrap border ${theme === 'dark'
                                ? 'bg-[#131314] text-white border-[#8e918f] hover:bg-[#1e1e1f]'
                                : 'bg-white text-[#1f1f1f] border-[#747775] hover:bg-[#f8f9fa]'
                                }`}
                        >
                ...
                {isAdding && (
                    <form onSubmit={handleAdd} className="bg-secondary/40 backdrop-blur-xl rounded-2xl border border-border/50 p-5 space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-medium text-muted-foreground">Flavour Notes / Feedback</label>
                            <textarea
